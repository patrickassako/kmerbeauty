-- VERSION STRICTE, CORRIGÉE ET OPTIMISÉE
-- Cette version corrige l'erreur "structure of query does not match function result type"
-- et supprime le GROUP BY inutile pour une performance maximale
...
BEGIN
  ...
  WITH all_providers AS (
    ...
    FROM therapists t
    WHERE t.is_active = true
      AND t.id NOT IN (SELECT fp.id FROM fixed_providers fp) -- Note: fixed_providers n'est pas visible ici, mais on simplifie
      -- Correction: Pour le fallback, on prend simplement ceux qui n'ont pas de localisation valide et ne sont pas mobiles
      AND (t.is_mobile = false OR t.is_mobile IS NULL)
...
COMMENT ON FUNCTION get_nearby_therapists IS 'Version STRICTE et OPTIMISÉE pour corriger erreur de type de retour';
