-- VERSION 7: UNIFIED PROVIDERS SEARCH (Therapists + Salons)
-- Recherche unifiée avec filtrage par Service ID et tri intelligent
-- Priorité: Quartier > Ville > Proximité
...
CREATE OR REPLACE FUNCTION get_nearby_providers(
  ...
  radius_meters INTEGER DEFAULT 30000, -- Reduced default radius to 30km
...
RETURNS TABLE (
  ...
  latitude NUMERIC,
  longitude NUMERIC,
  service_price NUMERIC, -- Prix spécifique au service
  match_type TEXT,
  match_score INTEGER
)
LANGUAGE plpgsql
...
COMMENT ON FUNCTION get_nearby_providers IS 'Recherche unifiée (Thérapeutes + Salons) par Service ID avec tri intelligent';
