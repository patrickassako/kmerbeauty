...
// Pages that don't need the countdown (auth pages, etc.)
...
export default function CountdownWrapper({ children }: CountdownWrapperProps) {
    ...
    // Show children if:
    // 1. User is logged in
    // 2. Launch has happened
    // 3. Current path is excluded (login, signup, etc.)
    if (isLoggedIn || isLaunched || isExcludedPath) {
        return <>{children}</>;
    }
