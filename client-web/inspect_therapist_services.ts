...
async function inspectTherapistServices() {
    console.log("--- Therapist Services ---");
    const { data, error } = await supabase.from('therapist_services').select('*').limit(1);
    if (error) console.error(error);
    else console.log(JSON.stringify(data, null, 2));
}

inspectTherapistServices();
